...
public class ManageServer {
    ...
    private void exeCmd(String cmdType) {
        if (CmdType.LIST.getCmd().equals(cmdType)) {
            execList();
        } else if (CmdType.CREATE.getCmd().equals(cmdType)) {
            execCreateRecursion();
        } else if (CmdType.MODIFY.getCmd().equals(cmdType)) {
            execModify();
        } else {
    ...
    // 创建config节点
    private void execCreateRecursion() {
    ...
    private void execModify() {
        ...
        try {
            ...
            execCreateRecursion(); // 写入时config节点还未存在，则创建它
        }
    }
